...
class CustomerService {
    ...
    async processVerificationPhoto(
        ...
        // No profile photo → accept verification, send verification image to AI
        if (!profileCustomer?.profilePhotoKey) {
            sendPersonImagesToAI({ personKey, images: [photoDataUri] });
            return {
                status: "verified",
                matched: true,
                personKey,
        ...
        if (comparison.status === "verified") {
            // Send both profile photo and verification photo to AI for best recognition accuracy
            sendPersonImagesToAI({
                personKey,
                images: [
                    profileCustomer.profilePhotoKey,
